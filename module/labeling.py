...
def compute_iou(boxes1, boxes2):
    ...
    intersection = tf.where(tf.less_equal(intersection, 1e-8), 0.0, intersection)
...
# iou_matrix = compute_iou(anchor_boxes, gt_boxes[1])
# max_iou = tf.reduce_max(iou_matrix, axis=1)
# matched_gt_idx = tf.argmax(iou_matrix, axis=1)
# sum(matched_gt_idx.numpy() == 0)
...
# matched_gt_boxes = tf.gather(gt_boxes[1], matched_gt_idx)
# np.unique(matched_gt_boxes.numpy(), axis=0)
...
# matched_gt_cls_ids = tf.gather(cls_ids[1], matched_gt_idx)
# np.unique(matched_gt_cls_ids.numpy())
# cls_target = tf.where(
#     tf.not_equal(positive_mask, 1.0), -1.0, tf.cast(matched_gt_cls_ids, tf.float32)
# )
...
#     label = _encode_sample(images_shape, gt_boxes[i], cls_ids[i])
#     labels = labels.write(0, label)
# labels.stack()
    ...
    def _match_anchor_boxes(
        ...
        Arguments:
            ...
            gt_boxes: A float tensor with shape `(num_objects, 4)` representing
                the ground truth boxes, where each box is of the format `[x, y, width, height]`.
